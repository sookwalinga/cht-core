...
describe('Delivery in facility target tests', () => {
  ...
  it('Delivery in facility should be counted', async () => {
    ...
    let facilityDeliveries = await harness.getTargets({
      type: 'facility-deliveries'
    });
    ...
    const delivery = await harness.fillForm(
      'delivery',
      ...deliveryReportScenarios.oneChildHealthyFacility
    );
    expect(delivery.errors).to.be.empty;
    //await harness.setNow('1999-08-01');
    //await harness.flush(day);
    facilityDeliveries = await harness.getTargets({
      type: 'facility-deliveries'
    });
    expect(facilityDeliveries).to.have.property('length', 1);
    expect(facilityDeliveries[0]).to.nested.include({
      'value.pass': 1,
      'value.total': 1
    });
  });
  ...
  it('Delivery in home should not be counted as delivery in facility', async () => {
    ...
    const delivery = await harness.fillForm(
      'delivery',
      ...deliveryReportScenarios.oneChildHealthyHome
    );
    expect(delivery.errors).to.be.empty;
    //await harness.setNow('1999-08-01');
    //await harness.flush(day);
    const facilityDeliveries = await harness.getTargets({
      type: 'facility-deliveries'
    });

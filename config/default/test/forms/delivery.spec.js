  ...
  describe('PNC Visits', () => {
    ['within_24_hrs', '3_days', '7_days', 'none'].forEach(
      (pncVisitsAttended) => {
        it(`shows summary when the the PNC Visits attended is [${pncVisitsAttended}]`, async () => {
          ...
          expect(summary.r_pnc_visit_24hrs).to.equal(
            'within_24_hrs' === pncVisitsAttended ? '' : undefined
          );
          expect(summary.r_pnc_visit_3days).to.equal(
            '3_days' === pncVisitsAttended ? '' : undefined
          );
          expect(summary.r_pnc_visit_7days).to.equal(
            '7_days' === pncVisitsAttended ? '' : undefined
          );
          expect(summary.r_pnc_visit_6weeks).to.equal(
            '6_weeks' === pncVisitsAttended ? '' : undefined
          );
          expect(summary.r_pnc_visit_none).to.equal(
            'none' === pncVisitsAttended ? '' : undefined
          );
    ...
    ['alive_well', 'alive_unwell'].forEach((motherOutcome) => {
      it(`shows summary when the baby is deceased but the mother is [${motherOutcome}]`, async () => {
        const result = await harness.fillForm(
          'delivery',
          ...deliveryReportScenarios.babyDeceased(TODAY, motherOutcome)
        );
    ...
    it('shows summary when the mother is deceased but the baby is alive and no PNC visits have happened', async () => {
      const result = await harness.fillForm(
        'delivery',
        ...deliveryReportScenarios.motherDeceased(TODAY)
      );
    ...
    it('does not show summary when the baby and the mother are deceased', async () => {
      const result = await harness.fillForm(
        'delivery',
        ...deliveryReportScenarios.babyDeceased_motherDeceased(TODAY)
      );

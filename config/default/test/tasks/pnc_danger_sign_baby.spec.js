...
describe('PNC danger sign follow up for baby tests', () => {
  ...
  it('PNC danger sign follow up for baby from profile', async () => {
    ...
    const followupFormResult = await harness.fillForm(
      ...pncDangerSignFollowUpScenarios.baby.cured
    );
    expect(followupFormResult.errors).to.be.empty;
    expect(followupFormResult.report.fields.data.meta.__delivery_uuid).to.equal(
      newbornBaby.created_by_doc
    );
  ...
  it('PNC danger sign follow up from PNC danger sign follow up', async () => {
    ...
    let followupFormResult = await harness.fillForm(
      ...pncDangerSignFollowUpScenarios.baby.danger
    );
    expect(followupFormResult.report.fields.data.meta.__delivery_uuid).to.equal(
      newbornBaby.created_by_doc
    );

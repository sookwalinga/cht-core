...
function isFormArraySubmittedInWindowExcludingThisReport(
  reports,
  formArray,
  start,
  end,
...
const getLMPDateFromPregnancy = function (report) {
  return (
    isPregnancyForm(report) && getDateMS(getField(report, 'lmp_date_8601'))
  );
};
...
function getMostRecentLMPDateForPregnancy(contact, report) {
  ...
  getSubsequentPregnancyFollowUps(contact, report).forEach(function (v) {
    ...
    if (
      v.reported_date > mostRecentReportDate &&
      lmpFromPregnancyFollowUp !== '' &&
      lmpFromPregnancyFollowUp !== mostRecentLMP
    ) {
...
function isPregnancyTerminatedByAbortion(contact, report) {
  ...
  return (
    latestFollowup &&
    getField(latestFollowup, 'pregnancy_summary.visit_option') === 'abortion'
  );
}
...
function isPregnancyTerminatedByMiscarriage(contact, report) {
  ...
  return (
    latestFollowup &&
    getField(latestFollowup, 'pregnancy_summary.visit_option') === 'miscarriage'
  );
}
...
function isActivePregnancy(contact, report) {
  ...
  const lmpDate =
    getMostRecentLMPDateForPregnancy(contact, report) || report.reported_date;
  const isPregnancyRegisteredWithin9Months =
    lmpDate > today - MAX_DAYS_IN_PREGNANCY * MS_IN_DAY;
  const isPregnancyTerminatedByDeliveryInLast6Weeks =
    getSubsequentDeliveries(contact, report, 6 * 7).length > 0;
  const isPregnancyTerminatedByAnotherPregnancyReport =
    getSubsequentPregnancies(contact, report).length > 0;
  return (
...
function isPregnancyTaskMuted(contact) {
  ...
  return (
    latestVisit &&
    isPregnancyFollowUpForm(latestVisit) &&

...
module.exports = {
  ...
  isHighRiskPregnancy: function (c) {
    ...
    if (report && report.fields && report.fields.pregnant_woman_information) {
      ...
      if (report.fields.pregnant_woman_information.delivery_complications) {
        var deliveryComplications = report.fields.pregnant_woman_information.delivery_complications || '';
    ...
    if (report && report.fields && report.fields.rch_card) {
      ...
      if (report.fields.rch_card.medical_condition) {
        var medicalCondition = report.fields.rch_card.medical_condition || '';
  ...
  isHighRiskPregnancyML: function (c) {
    ...
    let firstVisit = this.getMostRecentConsentingPregnancyReport(c);
    let houseInfo = get(firstVisit, 'report.fields.education_and_prev_enrollment') || {};
    let prevDeliveryLocation = get(firstVisit, 'fields.facility_delivery_importance.delivery_location');
    let pregInfo = get(firstVisit, 'fields.pregnant_woman_information') || {};
    pregInfo.delivery_complications = pregInfo.delivery_complications || '';
    ...
    var inputData = [
      ...
      pregInfo.delivery_complications.includes('eclampsia') ? 1 : 0, //previous_eclampsia (no match) 
      pregInfo.delivery_complications.includes('large_perineal_tear') ? 1 : 0, //previous_perineal_tear (no match) 
      0,//previous_placenta_previa (no match)
      pregInfo.delivery_complications.includes('postpartum_hemorrage') ? 1 : 0,//previous_pph (no match) 
      pregInfo.delivery_complications.includes('prolonged_labor') ? 1 : 0,//previous_prolonged_labor (no match) 
      pregInfo.delivery_complications.includes('retained_placenta') ? 1 : 0, //previous_retained_placenta (no match)
      pregInfo.previous_delivery_by_vacuum === 'yes' ? 1 : 0,//previous_vacuum (no match) 
      pregInfo.previous_delivery_by_c_section === 'yes' ? 1 : 0,//prior_c_section (no match) 
      this.getMedicalCondition(c, 'sickle_cell') ? 1 : 0, //sickle_cell (no match) 
      pregInfo.previous_stillbirth === 'yes' ? 1 : 0, // previous stillbirth
      home_ct, //home_ct 
      0, // Twins (No match) 
      ...
      houseInfo.waterSource === 'other' || houseInfo.waterSource === 'decline_to_answer', //Water source = Other 
      houseInfo.waterSource === 'surface',//Water source  surface 
      houseInfo.waterSource === 'home_tap_pump', //Water source home_tap_pump
      houseInfo.waterSource === 'community_tap_pump',//Water source community_tap_pump
      houseInfo.waterSource === 'well',//Water source well 
      houseInfo.waterSource === 'home_well',//Water source home well 
      houseInfo.highestSchoolLevel === 'primary',// education level === primary 
      houseInfo.highestSchoolLevel === 'secondary',//secondary 
      houseInfo.highestSchoolLevel === 'completed_high_school' ||
      houseInfo.highestSchoolLevel === 'more_than_high_school',// completed_high_school or more_than_high_school
      houseInfo.highestSchoolLevel === 'incomplete_primary',// incomplete_primary
      houseInfo.highestSchoolLevel === 'incomplete_secondary',//incomplete_secondary
      houseInfo.highestSchoolLevel === 'school_level_dont_know_decline' ||
      houseInfo.highestSchoolLevel === 'no_formal_education',//highest_school_level = "school_level_dont_know_decline" or highest_school_level = "no_formal_education"
      houseInfo.homeElectricity === 'no',//Electricity = 'no' 
      houseInfo.homeElectricity === 'yes', //Electricity = 'yes'
      houseInfo.floorMaterial === 'decline_to_answer' ||
      houseInfo.floorMaterial === 'other', // floor = "decline_to_answer" or "other"
      houseInfo.floorMaterial === 'concrete', // floor = concrete 
      houseInfo.floorMaterial === 'dirt', // floor = dirt 
      houseInfo.floorMaterial === 'plastic_mat', // floor = plastic_mat 
      houseInfo.floorMaterial === 'tiles', // floor = tiles 
      houseInfo.roofMaterial === 'decline_to_answer' ||
      houseInfo.roofMaterial === 'other',// roof = "decline_to_answer" or "other" 
      houseInfo.roofMaterial === 'iron_sheets', // roof = iron_sheets
      houseInfo.roofMaterial === 'scrap_iron', // roof = scrap_iron 
      houseInfo.roofMaterial === 'makuti', // roof = makuti 
      houseInfo.roofMaterial === 'tiles_shingles',  //roof = tiles_shingles 
      shehia === 'bambi'
      , shehia === 'bandamaji'
      , shehia === 'binguni'
      , shehia === 'bopwe'

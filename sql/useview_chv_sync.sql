...
(
  ...
  clinic_cte AS (
    ...
    FROM
      contactview_metadata AS clinic
    LEFT JOIN
      contactview_metadata AS chw_area
      ON
  ...
  persons_cte AS (
    ...
    FROM
      contactview_metadata AS person
    INNER JOIN
      clinic_cte AS clinic
      ON
  ...
  FROM
    (SELECT DISTINCT ON
      ...
      LEFT JOIN
        ...
        AS docs ON docs.doc_uuid = info.doc_uuid

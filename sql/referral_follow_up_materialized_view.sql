...
------------------------------------------------------------
CREATE MATERIALIZED VIEW referral_follow_up AS 
(
	SELECT
		doc ->> '_id' AS _id,
        doc ->> '_rev' AS _rev,
...
CREATE UNIQUE INDEX IF NOT EXISTS referral_follow_up_reported_date_created_by_uuid ON referral_follow_up USING btree (reported_date, chv_uuid, patient_id);
-- Permissions
REASSIGN OWNED BY current_user TO full_access;
GRANT SELECT ON household TO full_access;
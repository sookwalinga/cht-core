...
WITH daily_meet AS (
  SELECT
   date_trunc('day',cm.reported_date) AS day
   ,district
   ,sa.name as facility
  ...
  GROUP BY district,supervisory_area_uuid,facility,day
)
...
FROM daily_meet
GROUP BY district,month--,facility
);

CREATE UNIQUE INDEX IF NOT EXISTS agg_district_chv_supervisor_activities ON agg_supervisor_activities USING btree (month,district);--,facility);
-- Permissions

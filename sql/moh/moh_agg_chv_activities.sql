...
(
  SELECT
    ...
    sum(chv_conducted_group_counselling) AS chv_conducted_group_counselling
  FROM agg_chv_activities
  GROUP BY
    district, shehia, reported_month,
    substring(disaggregation, '(sex|maternal_age|default)'),
    substring(disaggregation_value, '((female|male)|\d+_\d+(?=years)|default)')
);
CREATE UNIQUE INDEX IF NOT EXISTS moh_district_month_shehia_moh_agg_chv_activities ON moh_agg_chv_activities USING btree (district, shehia, reported_month, disaggregation, disaggregation_value);

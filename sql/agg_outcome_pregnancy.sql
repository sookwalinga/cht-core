...
(
  ...
  outcomes AS (
    SELECT
      ...
      sum(coalesce((po.live_birth OR po.miscarriage_or_stillbirth = 'stillbirth')::INT, 0)) AS children_born,
      ...
      sum(coalesce((po.delivery_location IN ('home', 'facility', 'in_transit'))::INT, 0)) AS woman_delivery,
  ...
  FULL OUTER JOIN agg_client_records AS ce_records
    ON
      ce_records.end_month = outcomes.reported_month
